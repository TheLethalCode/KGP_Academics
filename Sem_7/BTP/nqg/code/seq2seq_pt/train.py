...
def IPOT_distance2(C, b, n, m, beta=1, t_steps=20, k_steps=1):
    sigma = torch.ones(b, m, 1).float().cuda() / m # [b, m, 1]
    T = torch.ones(b, n, m).float().cuda() # [b, n, m]
    A = torch.exp(-C / beta).float() # [b, n, m]
    for t in range(t_steps):
        Q = A * T # [b, n, m]
        for k in range(k_steps):
            delta = 1 / (n * torch.matmul(Q, sigma)) # [b, n, 1]
            sigma = 1 / (m * torch.matmul(torch.transpose(Q, 1, 2), delta)) # [b, m, 1]
        T = delta * Q * torch.transpose(sigma, 1, 2) # [b, n, m]
    distance = torch.einsum('bii->b', torch.matmul(torch.transpose(C, 1, 2), T)) #[b,] Calculates batch trace
    return distance
...
def trainModel(model, translator, trainData, validData, dataset, optim):
    ...
    def trainEpoch(epoch):
        ...
        for i in range(len(trainData)):
            ...
            model.zero_grad()
            # ipdb.set_trace()
            g_outputs, c_outputs, c_gate_values, src_words = model(batch)
            ...
            # cosine cost
            cosine_cost = 1 - torch.bmm(c_outputs.permute(1, 0, 2), src_words.permute(1, 0, 2))
